...
class Molden(object):
    ...
    def gwfn(self, f='gwfn.data'):
        ...
        with open(f, 'w') as gwfn:
            result = {}
            result.update(self.gwfn_title())
            result.update(self.gwfn_basic_info())
            result.update(self.gwfn_geometry())
            result.update(self.gwfn_basis_set())
            result.update(self.gwfn_orbital_coefficients())
            gwfn.write(gwfn_template.format(**result))
    ...
    def gwfn_basic_info(self):
        ...
        return dict(
            version=__version__,
            spin_restriction='.true.' if self.spin_unrestricted() else '.false.',
    ...
    def gwfn_geometry(self):
        ...
        """
        result = dict(natom=self.natom())

        result['positions'] = '\n'.join(("% .13E% .13E% .13E" % (atom['X'], atom['Y'], atom['Z']) for atom in self.atom_list))
        ...
        for num, atom in enumerate(self.atom_list):
            if num % 8 == 0 and num > 0:
                result['atomic_list'] += "\n"
            if atom['pseudo']:
                result['atomic_list'] += "%10d" % (atom['N'] + 200)
            else:
                result['atomic_list'] += "%10d" % atom['N']

        result['valence_charges'] = ''
        for num, atom in enumerate(self.atom_list):
            if num % 4 == 0 and num > 0:
                result['valence_charges'] += "\n"
            result['valence_charges'] += " %1.13E" % self.charge(atom)
    ...
    def gwfn_basis_set(self):
        ...
        for atom in self.atom_list:
            for shell in atom['SHELLS']:
                if num % 8 == 0 and num > 0:
                    result['shell_types'] += "\n"
                result['shell_types'] += "%10d" % ang_type_map[shell['TYPE']]
                num += 1
        ...
        for atom in self.atom_list:
            for shell in atom['SHELLS']:
                if num % 8 == 0 and num > 0:
                    result['primitives'] += "\n"
                result['primitives'] += "%10d" % len(shell['DATA'])
                num += 1
        ...
        for num, atom in enumerate(dummy_atom + self.atom_list):
            ...
            if num % 8 == 0 and num > 0:
                result['first_shell'] += "\n"
            result['first_shell'] += "%10d" % sequence_number
        ...
        for atom in self.atom_list:
            for shell in atom['SHELLS']:
                for primitive in shell['DATA']:
                    if num % 4 == 0 and num > 0:
                        result['exponents'] += "\n"
                    result['exponents'] += "% .13E" % primitive[0]
                    num += 1

        num = 0
        result['contraction coefficients'] = ''
        for atom in self.atom_list:
            for shell in atom['SHELLS']:
                for primitive in shell['DATA']:
                    if num % 4 == 0 and num > 0:
                        result['contraction coefficients'] += "\n"
                    result['contraction coefficients'] += "% .13E" % primitive[1]
                    num += 1

        shell_positions = []
        for atom in self.atom_list:
            for _ in atom['SHELLS']:  # throwaway variable
                shell_positions.append("% .13E% .13E% .13E" % (atom['X'], atom['Y'], atom['Z']))
        result['shell_positions'] = '\n'.join(shell_positions)
    ...
    def gwfn_orbital_coefficients(self):
        ...
        """
        result = dict(orbital_coefficients='')
        ...
        for orbital in sorted(self.mo_matrix, key=itemgetter('SPIN', 'ENERGY')):
            for ao in orbital['MO']:
                for coefficient in ao['DATA']:
                    ...
                    result['orbital_coefficients'] += "% .13E" % coefficient
                    num += 1
